...
def _compute_taux_bdt(
    ...
    def _prog(done: int, total: int, n_cached: int, n_fetch: int, eta_seconds: float = 0.0) -> None:
        pct = min(done / max(total, 1), 1.0)
        eta_s = max(int(eta_seconds), 0)
        eta_txt = f"{eta_s // 60}m {eta_s % 60:02d}s"
        progress.progress(pct, text=f"Cache: {n_cached} | Reseau: {n_fetch} | {done}/{total} | ETA: {eta_txt}")
